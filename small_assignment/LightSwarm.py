...
def get_time_as_master(current_time, masters, timestamps, colormap):
    ...
    verifier = verifier + [masters[i] != masters[i-1] for i in range(1, len(masters) - 1)]
       
    if len(masters) > 1:

        if masters[-1] == masters[-2]:
    ...
    # Filter the masters and timestamps arrays to only include moments when the master has changed
 
    filtered = [(m, t) for (m, t, v) in zip(masters, timestamps, verifier) if v]
        
    # Get the deltas between which the master has changed for each master

    deltas = [(filtered[i][0], filtered[i + 1][1] - filtered[i][1]) for i in range(len(filtered) - 1)]
    deltas.append((filtered[-1][0], current_time - filtered[-1][1]))

    # Get the total time each node has spent as master and add the corresponding color
    ...
    for m, d in deltas:
        ...
        if pos != -1:
            ...
            bar_values.append([m, d, colormap[m]])

    # Sort the bars so that they do not change positions

    bar_values.sort(key=lambda tup: tup[0])

    # Unzip the values to get the correct format
...
def get_line(current_time, masters, timestamps, data, colormap):
    ...
    # Merge the x and y points, as well as the color in one array.

    points = [(timestamps[i] - current_time, data[i], colormap[masters[i]]) for i in range(len(timestamps))]

    # Collect the each point an its neighbor to represent a segment and assign to it a color 
...
def parseLogPacket(message):
    ...
    MASTERS.append(message[2])
    TIMESTAMPS.append(time.time())
    DATA.append(int(data[0][3]))
