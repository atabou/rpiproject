        ...
        if (swarmElement[4] == "PR"):
            ...
            if (swarmElement[1] == "1"):
                ...
                for j in range(SWARMSIZE):
                    ...
                    if int(swarmList[j].split(",")[5]) != 0 and int(swarmList[j].split(",")[5]) < int(swarmList[min].split(",")[5]):
                        min = j

                    if int(swarmList[j].split(",")[5]) > int(swarmList[max].split(",")[5]):
                        max = j

                if min == i:
                    led_to_blink = GLED
                    led_speed = (1536 - 256 * (int(swarmElement[3]) // 256)) / 1000.0
                elif max == i:
                    led_to_blink = RLED
                    led_speed =  (1536 - 256 * (int(swarmElement[3]) // 256)) / 1000.0
                else:
...
def gyr_blinking():
    ...
    while(True):
        ...
        if led_to_blink is not None:
            GPIO.output(led_to_blink, led_state)
            led_state = not led_state

        time.sleep(led_speed / 2)
...
try:
    ...
    ryg_thread = threading.Thread(target=gyr_blinking)
